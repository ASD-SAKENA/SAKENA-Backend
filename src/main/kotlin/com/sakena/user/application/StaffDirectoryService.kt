 ...
 * request to a worker. Staff are a system-wide pool, not scoped to a
 * building, so every manager sees the same list — but only staff, unlike the
 * admin-only [UserAdminService] which sees every account.
 */
@Service
